...
export default function MapaScreen() {
  ...
  useEffect(() => {
    if (!loading && stations.length > 0 && webRef.current) {
      const script = `
        if (window.renderEstacoes) window.renderEstacoes(${JSON.stringify(stations)});
        true;
      `;
...
<script>
  ...
  L.marker([${DEFAULT_LAT}, ${DEFAULT_LNG}], { icon: userIcon }).addTo(map);
  window.renderEstacoes = function(estacoes) {
    ...
    <View style={s.root}>
      ...
      <View style={s.header}>
        <View style={s.headerCard}>
          ...
          <Text style={s.headerSub}>{loading ? 'Carregando...' : `${stations.length} encontradas · Porto Alegre`}</Text>
        </View>
      </View>
        ...
        <Animated.View style={[s.cardWrap, { transform:[{translateY: cardY}] }]}>
          <View style={s.card}>
            ...
            <View style={s.cardHeader}>
              <View style={[s.statusBadge, { backgroundColor: estacao.conectores_livres > 0 ? 'rgba(0,255,135,0.15)' : 'rgba(255,184,0,0.15)' }]}>
                <View style={[s.statusDot, { backgroundColor: estacao.conectores_livres > 0 ? '#00FF87' : '#FFB800' }]} />
                <Text style={[s.statusText, { color: estacao.conectores_livres > 0 ? '#00FF87' : '#FFB800' }]}>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  navigation.navigate('StationDetail', {
                    estacao: {
                      id: estacao.id, nome: estacao.nome, end: estacao.endereco,
                      dist: '—', potencia: `${estacao.potencia_kw}kW`, tipo: estacao.tipo,
                      status: estacao.conectores_livres > 0 ? 'livre' : 'ocupado',
                      preco: `R$ ${estacao.preco_kwh}/kWh`, rating: 4.8,
                      vagas: estacao.conectores_livres, lat: estacao.latitude, lng: estacao.longitude,
                    }
                  });
                }}>
                <Text style={s.btnRecarregarText}>{estacao.conectores_livres > 0 ? '⚡ Ver Detalhes' : 'Indisponível'}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </Animated.View>
