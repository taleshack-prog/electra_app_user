...
export default function CadastroScreen() {
  ...
  const handleCadastro = async () => {
    ...
    if (!termos) {
      Alert.alert('Termos', 'Aceite os termos de uso para continuar.');
      return;
    }
    setLoading(true);
    ...
    <KeyboardAvoidingView style={styles.root} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <Animated.View style={[styles.titleArea, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
          <Text style={styles.title}>Bem-vindo ao{'\n'}<Text style={styles.titleCyan}>ELECTRA</Text></Text>
        ...
        <Animated.View style={[styles.form, { opacity: fadeAnim }]}>
          ...
          <View style={styles.inputWrap}>
            ...
            <View style={[styles.inputBox, nomeFocused && styles.inputFocused]}>
              ...
              <TextInput style={styles.input} placeholder="João Silva" placeholderTextColor="rgba(240,244,255,0.2)"
          ...
          <View style={styles.inputWrap}>
            ...
            <View style={[styles.inputBox, emailFocused && styles.inputFocused]}>
              <Text style={styles.inputIcon}>✉</Text>
              <TextInput style={styles.input} placeholder="seu@email.com" placeholderTextColor="rgba(240,244,255,0.2)"
          ...
          <View style={styles.inputWrap}>
            ...
            <View style={[styles.inputBox, telFocused && styles.inputFocused]}>
              ...
              <TextInput style={styles.input} placeholder="+55 (11) 99999-9999" placeholderTextColor="rgba(240,244,255,0.2)"
                value={tel} onChangeText={setTel}
                onFocus={() => setTelFocused(true)} onBlur={() => setTelFocused(false)}
          ...
          <View style={styles.inputWrap}>
            ...
            <View style={[styles.inputBox, senhaFocused && styles.inputFocused]}>
              <Text style={styles.inputIcon}>🔒</Text>
              <TextInput style={styles.input} placeholder="Mínimo 6 caracteres" placeholderTextColor="rgba(240,244,255,0.2)"
                value={senha} onChangeText={setSenha}
                onFocus={() => setSenhaFocused(true)} onBlur={() => setSenhaFocused(false)}
                secureTextEntry={!senhaVisible} autoCapitalize="none" />
            ...
            {senha.length > 0 && (
              <View style={styles.forceWrap}>
                <View style={styles.forceBars}>
                  {[1,2,3,4].map(i => (
                    <View key={i} style={[styles.forceBar, { backgroundColor: i <= force ? forceColor[force] : 'rgba(255,255,255,0.08)' }]} />
                  ))}
                </View>
                <Text style={[styles.forceLabel, { color: forceColor[force] }]}>{forceLabel[force]}</Text>
              </View>
            )}
          </View>
          ...
          <TouchableOpacity style={styles.termosRow} onPress={() => setTermos(t => !t)}>
            ...
            <Text style={styles.termosText}>
              Li e aceito os <Text style={styles.termosLink}>Termos de Uso</Text> e a <Text style={styles.termosLink}>Política de Privacidade</Text>
            </Text>
          </TouchableOpacity>

          <TouchableOpacity
            style={[styles.btnPrimary, (!termos || loading) && styles.btnDisabled]}
            activeOpacity={termos ? 0.85 : 1}
            onPress={handleCadastro}
            disabled={!termos || loading}
          >
            <Text style={styles.btnText}>{loading ? 'Criando conta...' : 'Criar minha conta'}</Text>
